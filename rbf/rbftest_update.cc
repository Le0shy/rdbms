...
int RBFTest_Update(RecordBasedFileManager *rbfm)
{
    ...
    /* add test for read attribute */
    string attribute_name = "EmpName";
    const char* attribute = longstr.c_str();
    char* returned_attribute = new char[2000];
    rc = rbfm ->readAttribute(fileHandle, recordDescriptor, shortRID,
                              attribute_name,returned_attribute);
    assert(rc == success && "readAttribute should success");
    assert(memcmp(attribute, returned_attribute + 5, 1000) == 0 && "Returned Data should be the same");
    delete[] returned_attribute;
